...
namespace VoroBidsCrm.Application.Services
{
        ...
        IUnitOfWork unitOfWork) : IPublicBookingService
    {
        ...
        public async Task<IEnumerable<VoroBidsCrm.Application.DTOs.CRM.AvailabilitySlotDto>> GetAvailableSlotsAsync(string tenantSlug, DateTime date, Guid? serviceId = null, Guid? employeeId = null)
        {
            var tenant = await tenantRepository.GetBySlugAsync(tenantSlug);
            ...
            var slots = new List<VoroBidsCrm.Application.DTOs.CRM.AvailabilitySlotDto>();
            var current = startOfDay.ToUniversalTime();

            // Bids-only Mode: If no active employees exist, treat the salon as a single resource with capacity 1
            ...
            while (current < endOfDay)
            {
                ...
                {
                    // For "Any professional" or Bids-only mode, busy only if ALL (or the default 1) capacity is occupied
                ...
                slots.Add(new VoroBidsCrm.Application.DTOs.CRM.AvailabilitySlotDto(current.ToUniversalTime(), next.ToUniversalTime(), !isBusy));
